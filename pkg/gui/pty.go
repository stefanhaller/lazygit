...
func (gui *Gui) newPtyTask(view *gocui.View, cmd *exec.Cmd, prefix string) error {
	...
	// that doesn't understand it ignores it, so we can set it always. Like
	// LAZYGIT_COLUMNS it has to be set before the no-pty path below, since on
	// that path git renders the diff itself, and git speaks the protocol too,
	// for its word-diff formats, whose markup we could not otherwise resolve.
